...
Detection priority:
  ...
  2. YOLO orange  → rotation reference
...
def detect_yolo(model, frame) -> dict:
    """
    Run YOLO inference.
    Returns dict with best detection per class:
      center: (cx, cy, area, sat, r) or None
      orange: (cx, cy, area, sat, r) or None
      red:    (cx, cy, area, sat, r) or None
      yellow: (cx, cy, area, sat, r) or None
      green:  (cx, cy, area, sat, r) or None
    """
    empty = {c: None for c in ["center","orange"]}
    if model is None:
        return empty
    ...
    try:
        ...
        for box in results.boxes:
            ...
            if name not in best or conf > best[name]["conf"]:
                best[name] = {"blob": (cx,cy,area,0,r), "conf": conf,
...
def draw_info_bar(bar, W, t_sec, rot_deg, rot_rad, cum_deg, cum_rad,
    ...
    dot(conf==100 or conf==85,c2+8,146)
    cv2.putText(bar,"Orange found" if (conf==100 or conf==85) else "Orange missing",
                (c2+22,151),cv2.FONT_HERSHEY_SIMPLEX,0.40,
                WHITE if (conf==100 or conf==85) else GRAY,1)
    if source:
...
def process(video_path: str, out_path: str, job_id: str = "local",
    ...
    # Pair fallback state (disabled but kept)
    gap_yel      = 0
    last_ya      = None
    last_yb      = None
    active_color = None
    ...
    while True:
        ...
        # ── YOLO ──────────────────────────────────────────────────────────
        yolo_det = detect_yolo(yolo, preproc)

        # ── OpenCV ────────────────────────────────────────────────────────
        cv_ora = find_blobs(hsv_src, markers["ORANGE"], max_y)

        # Pair fallback (disabled) — still computed, just not used for hub
        ...
        # ── Hub source: YOLO center >> pair fallback ───────────────────────
        hub_px_raw = None

        if yolo_det["center"] is not None:
            # YOLO found wheel center directly
            ...
            # Gap fill for YOLO center
            gap_hub   += 1
            hub_px_raw = last_hub

        # ── Orange source: YOLO >> OpenCV ─────────────────────────────────
        ...
        # ── Stable hub (running average) ───────────────────────────────────
        if has_hub:
            hub_buf.append(np.array(hub_px_raw))
            if len(hub_buf) > 30: hub_buf.pop(0)
            hub_stable = np.mean(hub_buf, axis=0)
            # hub_radius: from YOLO center bbox or from orange distance
            if yolo_det["center"] is not None:
                _,_,area,_,r = yolo_det["center"]
        ...
        # YOLO detections — draw all found classes
        yolo_colors = {
            "center": (255,255,255),
            "orange": (0,140,255),
            "red":    (40,40,220),
        ...
        for cls_name, blob in yolo_det.items():
            if blob is None: continue
            cx,cy,area,_,r = blob
            col = yolo_colors.get(cls_name,(180,180,180))
            cv2.circle(ann,(cx,cy),r+4,col,2,cv2.LINE_AA)
            cv2.putText(ann,f"Y:{cls_name}",(cx+r+4,cy+5),
        ...
        if hub_stable is not None:
            ...
            cv2.circle(ann,(int(hub_stable[0]),int(hub_stable[1])),
                       7,hcol,-1,cv2.LINE_AA)
            cv2.circle(ann,(int(hub_stable[0]),int(hub_stable[1])),
                       9,(0,0,0),1,cv2.LINE_AA)

        # Orange neon circle + line to hub
