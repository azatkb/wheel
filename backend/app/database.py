    ...
    try:
        ...
        record = {
            ...
            "physics_json":  json.dumps({
                ...
                "lajtner_time":     result.get("lajtner_time"),
                "lajtner_jerk_deg": result.get("lajtner_jerk_deg"),
                "lajtner_jerk_rad": result.get("lajtner_jerk_rad"),
            }),
        }
        # Replace any existing row for this job (so reprocessing UPDATES the
        # data instead of failing on a duplicate job_id — this is what makes
        # reprocessed LT/LJ actually land in the DB).
        try:
            sb.table("physics_results").delete().eq("job_id", job_id).execute()
        except Exception as _e:
            log.warning(f"[DB] Physics pre-delete failed (continuing): {_e}")
